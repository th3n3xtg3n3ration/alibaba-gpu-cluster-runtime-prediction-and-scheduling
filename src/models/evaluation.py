...
def evaluate_regression(
    ...
    dict
        ...
        - ``mape``  : Mean Absolute Percentage Error, expressed on a 0-100
          ...
          Only computed over samples where ``y_true > 0``
    ...
    UserWarning
        ...
        collapsed. The metrics are still returned and the caller decides what
    ...
    # MAPE: guard against zero-runtime jobs, which would produce inf or nan.
    # Scaled by 100 to report a genuine percentage, matching notebook 05's own
    # convention. The raw scikit-learn-style ratio understates the true
    # percentage by 100x.
    ...
    #
    # A model can collapse to a single constant output and still land an
    # ordinary MAE: CNN-LSTM Numeric Sequence predicted 4128.12 s for all 16,437
    # test jobs, scoring MAE 6717 s and R2 -0.01, mid-pack among the working
    # models. Nothing in mae, rmse, r2, mape or mdae separates that from a model
    # that learned something, while downstream the difference is total: a
    # constant prediction imposes no ordering, so an SJF policy built on it
    # degenerates to arrival order and matches FIFO exactly.
    ...
    # The verdict is taken here, on the evidence this function has just
    # produced. Recording pred_std and pred_unique_frac alone left the collapse
    # detectable but undetected, with nothing downstream looking at them.
    ...
    # distinct values still orders the queue, and the simulator schedules such a
    # column without complaint. What it says is that almost nothing was fitted.
...
def _distinct_count(frac, n) -> int | None:
    ...
    # nothing, while 15 out of 16,437 is 0.0009 and orders coarsely but
    # genuinely. Without the sample count there is nothing to judge.
    if not n:
        return None
...
def _collect_verdicts(metrics: dict, judge) -> bool | None:
    ...
    # above, so finalize_dl_model keeps every seed's spread evidence aligned
    # with its 'seeds' list and each entry is judged on its own. A checkpoint
    # written before those keys existed carries neither and still answers None.
