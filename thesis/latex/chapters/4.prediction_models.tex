...
\hline
\multirow{3}{*}{A} & \multirow{3}{*}{ML} & RF & \multirow{3}{*}{Numeric only} & \multirow{3}{*}{Independent rows} \\
                   &                     & XGB       &                               & \\
                   &                     & LGBM      &                               & \\
\hline
\multirow{4}{*}{B} & \multirow{4}{*}{ML} & RF & \multirow{4}{*}{Numeric + Categorical} & \multirow{4}{*}{Independent rows} \\
                   &                     & XGB       &                               & \\
                   &                     & LGBM (Native)&                               & \\
                   &                     & LGBM (One-Hot)&                               & \\
\hline
...
XGB~\cite{chen2016xgboost} employs gradient boosting: one by one, additional decision trees are generated for an ensemble using the residual values of the previously generated ensemble as input. In addition to this standard technique, XGB uses both $L_1$ and $L_2$ regularization terms when generating trees; these help control overfitting due to feature noise, thereby allowing XGB to be used in conjunction with noisy data, whereas unregularized gradient boosting models cannot be so easily utilized.
...
LGBM~\cite{ke2017lightgbm} accelerates gradient boosting through two core methods: GOSS, which allows LGBM to focus computation on those training examples with the largest gradients; and EFB, which takes advantage of the sparsity of feature matrices to reduce the number of effective features. When combined, the above approaches enable training speeds much greater than XGB when working with very large datasets. Accuracy was found to be similar or better compared to XGB.

All hyperparameters were tuned to optimize performance using a multi-stage process identical to that of XGB, including the use of early stopping. The final configuration is shown in Table~\ref{tab:hyperparams}.
...
\hline
\multirow{3}{*}{A} & RF & \multirow{3}{*}{Numeric only} & \multirow{3}{*}{Independent rows} & n\_estimators=150, max\_depth=20, max\_features=0.7, max\_samples=0.8 \\ 
                   & XGB       & & & n\_estimators=400, lr=0.025, max\_depth=15, subsample=0.8 \\ 
                   & LGBM      & & & n\_estimators=1200, lr=0.075, num\_leaves=36, max\_depth=10 \\ 
\hline
\multirow{4}{*}{B} & RF & \multirow{4}{*}{Numeric + Categorical} & \multirow{4}{*}{Independent rows} & n\_estimators=150, max\_depth=20, max\_features=0.7 \\ 
                   & XGB       & & & n\_estimators=1000, lr=0.075, max\_depth=15, reg\_lambda=0.1 \\ 
                   & LGBM (Native) & & & n\_estimators=1700, lr=0.03, num\_leaves=73, max\_depth=10 \\ 
                   & LGBM (One-Hot)& & & n\_estimators=1200, lr=0.075, num\_leaves=36, max\_depth=10 \\ 
\hline
