...
In all experiments where One-Hot encoding was used (B, D, F) there were two categorical characteristics, user and gpu\_type. Together, these two characteristics have been expanded into 582 binary indicator columns. This produces a total of 591 input characteristic values.

LightGBM, which is able to process categorical data natively, has been passed the two categorical characteristics directly as categories. As such, d=11.
...
\hline
Random Forest  & 4{,}316 & 1{,}508 & 13{,}831 & 16.85 & 0.27 \\ 
XGBoost        & 4{,}808 & 1{,}894 & 13{,}965 & 19.02 & 0.26 \\ 
LightGBM       & 5{,}212 & 2{,}259 & 14{,}315 & 20.66 & 0.22 \\ 
\hline
\end{tabular}
\end{table}
...
Table~\ref{tab:expb-full} lists the results for Experiment B. As such, adding the One-Hot encoded \texttt{gpu\_type} and \texttt{user} variables as additional features increases the total number of dimensions in the feature space from nine to 591. This is also the biggest single improvement of all of the experiments. The performance of the best experiment (Experiment A) was improved by 0.24 when using a One-Hot encoded version of the \texttt{gpu\_type} and \texttt{user} variables (XGBoost; One-Hot), while LightGBM's ability to natively use categorical values ($d=11$) produces a competitive result that does not have the same dimensionality issues as the One-Hot encoded data.

\begin{table}[H]
\centering
\caption[Results for Experiment B]{Experiment B: tree-based models using One-Hot encoding for categorical features. One-Hot encoding is applied to \texttt{gpu\_type} and \texttt{user}. LightGBM Native uses its built-in handler for categorical features.}
\label{tab:expb-full}
\begin{tabular}{lrrrrr}
\hline
\textbf{Model} & \textbf{MAE (s)} & \textbf{MedAE (s)} & \textbf{RMSE (s)} & \textbf{MAPE (\%)} & \textbf{R\textsuperscript{2}} \\ 
\hline
XGBoost (One-Hot)        & \textbf{3{,}389} & \textbf{1{,}129} & \textbf{11{,}375} & \textbf{10.80} & \textbf{0.51} \\ 
LightGBM (Native Cat.)   & 4{,}106 & 1{,}510 & 12{,}147 & 14.55 & 0.44 \\ 
Random Forest (One-Hot)  & 4{,}187 & 1{,}888 & 12{,}380 & 15.57 & 0.42 \\ 
LightGBM (One-Hot)       & 4{,}388 & 1{,}644 & 13{,}055 & 14.43 & 0.35 \\ 
\hline
\end{tabular}
\end{table}
...
Table~\ref{tab:expd-full} presents the performance data for Experiment D. Adding one-hot encoded categorical variables increased the number of input dimensions from 9 to 591 ($w=1$), thus improving the $R^2$ score of each of the three deep learning models when compared to Experiment C. The LSTM model showed improvement in its $R^2$ statistic, from a value of 0.00 to 0.06, ; these statistics suggest that both \texttt{user} and \texttt{gpu\_type} provide useful information about the outcome variable for the LSTM. Nevertheless, the highest possible $R^2$ value obtained by any of the DL models was less than half that achieved by the best performing tree based model in Experiment B (maximum $R^2$ = .11 CNN-LSTM).
...
Table~\ref{tab:expe-full} presents the results for Experiment E. The sliding window technique extended the size of the sequences from $w=1$ to $w=10$. This was achieved by adding nine numerical features into the sequence ($d=9$) allowing each of the three deep learning models to see the ten most recent jobs. However, despite having a richer temporal history than the static models used in Experiment C, none of the three DL models were able to outperform the static models: Negative $R^2$ values are observed for all three models, demonstrating that there is insufficient temporal autocorrelation among consecutive jobs on the Alibaba trace as to be useful for training sequential (sequence-based) models.
...
Table~\ref{tab:expf-full} shows the outcomes for Experiment F. Experiment F represents a mix of the two worst aspects of Experiments D and E: the categorical feature set is high dimensional while the sequencing frame is identical to that of Experiment E. As expected in this case, CNN performs slightly better than it did in Experiment E ($R^2 = 0.04$ versus $-0.02$). However, LSTM performed significantly worse than in Experiment D ($R^2 = -0.27$). These outcomes indicate that the increased training time caused by the expanded dimensionality of the categorical feature set adversely impacts LSTM optimization. In addition, the CNN-LSTM hybrid also performed worse when compared to Experiment E.
