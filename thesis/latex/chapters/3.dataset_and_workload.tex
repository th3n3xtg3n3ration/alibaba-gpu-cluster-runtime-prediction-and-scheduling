...
After removing invalid records, the processed dataset has \textbf{82,184 valid GPU jobs} remaining from the 100,000 original records; 17,816 records with zero GPU demand, missing runtimes, or a non-positive duration have been eliminated. A single CSV file will serve as a deterministic data source for all further analysis, modeling, and simulation.
...
\clearpage
Figure~\ref{fig:gpu-vs-runtime} shows the connection between GPU requests and runtime for jobs, using 5,000 randomly selected samples, with both axes on a log scale. The scatter plot shows no simple linear correlation between GPU requests and runtime. The jobs that used a single GPU had runtimes ranging from 10\textsuperscript{1}\,s to 10\textsuperscript{5}\,s ($\approx$10\,s to $\approx$3.5 days). Higher GPU requests (6-8 GPUs) are rare but appear close to the 10\textsuperscript{4}\,s mark. This confirms that \texttt{gpu\_demand} alone is fundamentally insufficient for accurate runtime prediction, motivating the addition of other context-based features like \texttt{user}, \texttt{gpu\_type}, and cluster utilization metrics.
...
\clearpage
Figure~\ref{fig:arrival-heatmap} provides a day-of-week by hour-of-day analysis for job submissions. The color intensity of each cell indicates the number of jobs submitted. There was peak submission activity during daytime hours from Monday through Friday (rows 0-4). The most intense submissions were on Thursdays (row 3) at hour values 0-2, 7, and 10-14. Submission rates dropped significantly on weekends (rows 5-6) compared to submission rates during the week. This significant diurnal and weekly variability motivates the selection of \texttt{hour\_of\_day} and \texttt{day\_of\_week} as predictive variables.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{figures/nb01-fig06-arrival-heatmap.png}
\caption[Job arrival heatmap]{Heatmap of job arrivals per hour of the day and per day of the week (Monday = 0). The color intensity corresponds to the number of jobs submitted. Weekday daytime hours appear to be the most concentrated period for job submissions.}
...
Submission timing for jobs is very much related to human behavior. Jobs are most often submitted during peak working hours, then drop off significantly when people leave work. Additionally, there are differences in submission timing for jobs based upon whether it is a weekday or a weekend. Temporal features have been created based on the submission timestamp for each job to understand these behaviors:
...
\begin{itemize} 
    ...
    \item \texttt{day\_of\_week}: The day of the week (0-6, where 0 is Monday) when a job is submitted.
\end{itemize} 

These allow the models to recognize that both submission behavior and runtime distribution vary in a predictable manner throughout the day and week.
...
The use of \texttt{category} data types allows LGBM to handle both categorical variables natively, thus eliminating the need to perform one-hot encoding and the associated memory usage. All other model types (XGB, RF, and all DL architectures) require numerical data. Therefore, the two categorical variables must be encoded using one-hot encoding.
...
\texttt{day\_of\_week} & Numeric (0-6) & Day of the week (0 = Monday) \\ 
